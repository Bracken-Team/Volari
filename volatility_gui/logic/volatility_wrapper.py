...
from volatility3.framework import contexts, automagic, interfaces, plugins as framework_plugins
...
class VolatilityWrapper:
    ...
    def run_plugin(self, plugin_name: str, additional_config: Dict[str, Any] = None, progress_callback=None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = self._process_treegrid(treegrid, progress_callback)
    ...
    def cleanup_context(self):
        """Clean up the context to release memory."""
        if self.ctx:
            # Clear layer cache and config to release memory
            try:
                if hasattr(self.ctx, 'layers'):
                    self.ctx.layers.clear()
                if hasattr(self.ctx, 'config'):
                    self.ctx.config.clear()
            except Exception as e:
                vollog.warning(f"Error during context cleanup: {e}")
            self.ctx = None
        gc.collect()
